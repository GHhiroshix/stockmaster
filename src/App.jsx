...
function exportIncomingCSV(rows){
  const hdr="入庫日,JAN,商品名,入庫数量,仕入れ単価,仕入れ合計,メーカー,仕入れ先,備考";
  const r=rows.map(h=>[h.date,h.jan,`"${h.name}"`,h.qty,h.cost,h.totalCost,`"${h.maker||""}"`,`"${h.supplier||""}"`,`"${h.note||""}"`].join(","));
  dl("\uFEFF"+[hdr,...r].join("\n"),"incoming_history.csv");
}
function exportOutgoingCSV(rows){
  const hdr="出庫日,JAN,商品名,出庫数量,販売単価,合計売上,販売先・出荷先,備考";
  const r=rows.map(o=>[o.date,o.jan,`"${o.name}"`,o.qty,o.price,o.totalPrice,`"${o.destination||""}"`,`"${o.note||""}"`].join(","));
  dl("\uFEFF"+[hdr,...r].join("\n"),"outgoing_history.csv");
}
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        ...
        setScanning(true);
        controls=await reader.decodeFromVideoDevice(back?.deviceId,videoRef.current,(result)=>{if(result){onDetected(result.getText());controls?.stop();}});
...
function CategorySelect({cats,l1,l2,l3,onChange,inpS}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      <div><div style={{fontSize:11,color:"#8B949E",marginBottom:3}}>大分類</div>
        <select style={inpS} value={l1||""} onChange={e=>onChange(e.target.value,"","")}>
          <option value="">選択してください</option>
          {cats.map(c=><option key={c.id} value={c.id}>{c.emoji} {c.name}</option>)}
        </select>
      </div>
      <div><div style={{fontSize:11,color:"#8B949E",marginBottom:3}}>中分類</div>
      ...
      <div><div style={{fontSize:11,color:"#8B949E",marginBottom:3}}>小分類</div>
...
function ItemCard({item,cats,onEdit,onDelete}){
  ...
  return(
    <div style={{background:"#21262D",border:"1px solid #30363D",borderRadius:10,padding:14,marginBottom:10}}>
      ...
      {(l1||l3)&&<div style={{fontSize:11,color:"#8B949E",marginBottom:8}}>{l1&&<span>{l1.emoji} {l1.name}</span>}{l3&&<span style={{color:"#484F58"}}> › {l3.name}</span>}</div>}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:8}}>
        <div style={{background:"#1C2128",borderRadius:6,padding:"8px 10px"}}><div style={{fontSize:9,color:"#484F58",marginBottom:3}}>単価</div><div style={{fontFamily:"monospace",fontWeight:700,color:"#58A6FF",fontSize:14}}>{fmtY(item.price)}</div></div>
        <div style={{background:"#1C2128",borderRadius:6,padding:"8px 10px"}}><div style={{fontSize:9,color:"#484F58",marginBottom:3}}>在庫数</div><div style={{fontFamily:"monospace",fontWeight:700,fontSize:20}}>{item.qty}</div></div>
        <div style={{background:"#1C2128",borderRadius:6,padding:"8px 10px"}}><div style={{fontSize:9,color:"#484F58",marginBottom:3}}>粗利率</div><div style={{fontFamily:"monospace",fontWeight:700,color:mCol(m),fontSize:14}}>{m!==null?m+"%":"—"}</div></div>
      </div>
...
export default function App(){
  ...
  const [fOutTxt,setFOutTxt]=useState("");
  const [fOutFrom,setFOutFrom]=useState("");
  const [fOutTo,setFOutTo]=useState("");
  ...
  function confirmAdd(){
    ...
    if(cost>0){
      const rec={id:Date.now(),date:today(),jan:modal.product.jan,name:modal.product.name,qty:arQty,cost,totalCost:arQty*cost,maker:arMaker,supplier:arSupplier,note:""};
      setIncoming(h=>[rec,...h]);
    }
  ...
  function confirmAddIncoming(){
    ...
    if(!incomingModal.name.trim()){addToast("商品名を入力してください","err");return;}
    const rec={...incomingModal,id:Date.now(),totalCost:(incomingModal.qty||0)*(incomingModal.cost||0)};
    setIncoming(h=>[rec,...h]);
    setIncomingModal(null);addToast("入庫履歴を追加しました","ok");
  }

  function confirmAddOutgoing(){
    if(!outgoingModal)return;
    if(!outgoingModal.name.trim()){addToast("商品名を入力してください","err");return;}
    if(!outgoingModal.qty||outgoingModal.qty<1){addToast("出庫数量を入力してください","err");return;}
    const item=db.find(i=>i.jan===outgoingModal.jan);
    if(item&&item.qty<outgoingModal.qty){
      if(!window.confirm("在庫が不足しています（現在"+item.qty+"個）。このまま出庫しますか？"))return;
    }
    if(item){
      setDb(d=>d.map(i=>i.jan===outgoingModal.jan?{...i,qty:Math.max(0,i.qty-outgoingModal.qty)}:i));
    }
    ...
    setOutgoing(o=>[rec,...o]);
    setOutgoingModal(null);addToast("出庫を記録しました ("+outgoingModal.qty+"個)","ok");
  }
  ...
  const inRows=useMemo(()=>incoming.filter(h=>{
    if(fInFrom&&h.date<fInFrom)return false;
    if(fInTo&&h.date>fInTo)return false;
    if(fInTxt){const t=fInTxt.toLowerCase();if(!h.name.toLowerCase().includes(t)&&!h.jan.includes(t)&&!(h.supplier||"").toLowerCase().includes(t))return false;}
    return true;
  }).sort((a,b)=>b.date.localeCompare(a.date)),[incoming,fInTxt,fInFrom,fInTo]);

  const outRows=useMemo(()=>outgoing.filter(o=>{
    if(fOutFrom&&o.date<fOutFrom)return false;
    if(fOutTo&&o.date>fOutTo)return false;
    if(fOutTxt){const t=fOutTxt.toLowerCase();if(!o.name.toLowerCase().includes(t)&&!o.jan.includes(t)&&!(o.destination||"").toLowerCase().includes(t))return false;}
    return true;
  }).sort((a,b)=>b.date.localeCompare(a.date)),[outgoing,fOutTxt,fOutFrom,fOutTo]);
  ...
  function EditCell(item,field,display,width,mono){
    ...
    if(editItem&&editItem.id===item.id&&editItem.field===field){
      return<input autoFocus type={isNum?"number":"text"} defaultValue={item[field]} style={{...eInp,width:width||80}} onBlur={e=>saveEdit(item.id,field,e.target.value)} onKeyDown={e=>{if(e.key==="Enter")saveEdit(item.id,field,e.target.value);if(e.key==="Escape")setEditItem(null);}}/>;
    }
    ...
    return(
      <div style={{background:"#161B22",borderBottom:"1px solid #30363D",padding:12}}>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          <input style={{...inpS,width:130}} type="date" value={from} onChange={e=>setFrom(e.target.value)}/>
          <span style={{color:"#484F58",fontSize:12}}>〜</span>
          <input style={{...inpS,width:130}} type="date" value={to} onChange={e=>setTo(e.target.value)}/>
          <input style={{...inpS,flex:1,minWidth:120}} type="text" placeholder={placeholder} value={txt} onChange={e=>setTxt(e.target.value)}/>
          {(from||to||txt)&&<button style={btnG} onClick={()=>{setFrom("");setTo("");setTxt("");}}>クリア</button>}
        </div>
      </div>
    );
  }
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",background:"#0D1117",color:"#E6EDF3",fontFamily:"system-ui,sans-serif",fontSize:14}}>
          ...
          <div style={{background:"#161B22",borderBottom:"1px solid #30363D",padding:12}}>
            <div style={{display:"flex",gap:8,maxWidth:600}}>
              ...
              <input ref={ref} style={{background:"#1C2128",border:"1px solid #30363D",borderRadius:6,color:"#E6EDF3",fontFamily:"monospace",fontSize:14,padding:"8px 10px",outline:"none",flex:1,letterSpacing:1}}
                type="text" inputMode="numeric" placeholder="JANコード手入力" value={jan}
      ...
      {tab==="incoming"&&(
        <div style={{display:"flex",flexDirection:"column",flex:1,minHeight:0}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px 6px"}}>
            <div style={{fontSize:14,fontWeight:700}}>📥 入庫履歴 <span style={{fontSize:12,color:"#484F58",fontWeight:400}}>({inRows.length}件 / 合計 <span style={{color:"#3FB950",fontWeight:600}}>{fmtY(inTotalCost)}</span>)</span></div>
            <div style={{display:"flex",gap:6}}>
              ...
              <button style={{background:"rgba(63,185,80,.12)",color:"#3FB950",border:"1px solid rgba(63,185,80,.3)",borderRadius:6,cursor:"pointer",fontWeight:600,fontSize:12,padding:"6px 10px"}} onClick={()=>{exportIncomingCSV(inRows);addToast("CSV出力しました","ok");}}>⬇ CSV</button>
            </div>
          </div>
          <div style={{flex:1,overflow:"auto",padding:"0 16px 20px"}}>
            {inRows.length===0?<div style={{padding:"40px 20px",textAlign:"center",color:"#484F58"}}><div style={{fontSize:40,marginBottom:8}}>📥</div>入庫履歴がありません</div>:(
              isMobile?(
                <div>{inRows.map(h=>(
                  <div key={h.id} style={{background:"#21262D",border:"1px solid #30363D",borderRadius:10,padding:14,marginBottom:10}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                      <span style={{fontFamily:"monospace",fontSize:12,color:"#3FB950",fontWeight:600}}>📥 {h.date}</span>
                      <button style={btnD} onClick={()=>{setIncoming(hist=>hist.filter(x=>x.id!==h.id));addToast("削除しました","info");}}>削除</button>
                    </div>
                ...
                <div style={{background:"#21262D",borderRadius:8,border:"1px solid #30363D",overflow:"hidden",marginTop:4}}>
                  <table style={{width:"100%",borderCollapse:"collapse",minWidth:700}}>
                    <thead><tr style={{background:"#1C2128"}}>{["入庫日","商品名","JANコード","入庫数","仕入れ単価","仕入れ合計","メーカー","仕入れ先","備考",""].map(h=><th key={h} style={thS}>{h}</th>)}</tr></thead>
                    <tbody>{inRows.map(h=>(
                      <tr key={h.id}>
                        <td style={{...tdS,fontFamily:"monospace",color:"#3FB950",fontWeight:600,whiteSpace:"nowrap"}}>{h.date}</td>
                        ...
                        <td style={tdS}><button style={btnD} onClick={()=>{setIncoming(hist=>hist.filter(x=>x.id!==h.id));addToast("削除しました","info");}}>削除</button></td>
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
      ...
      {tab==="outgoing"&&(
        <div style={{display:"flex",flexDirection:"column",flex:1,minHeight:0}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 16px 6px"}}>
            <div style={{fontSize:14,fontWeight:700}}>📤 出庫履歴 <span style={{fontSize:12,color:"#484F58",fontWeight:400}}>({outRows.length}件 / 合計売上 <span style={{color:"#58A6FF",fontWeight:600}}>{fmtY(outTotalPrice)}</span>)</span></div>
            <div style={{display:"flex",gap:6}}>
              ...
              <button style={{background:"rgba(88,166,255,.12)",color:"#58A6FF",border:"1px solid rgba(88,166,255,.3)",borderRadius:6,cursor:"pointer",fontWeight:600,fontSize:12,padding:"6px 10px"}} onClick={()=>{exportOutgoingCSV(outRows);addToast("CSV出力しました","ok");}}>⬇ CSV</button>
            </div>
          </div>
          <div style={{flex:1,overflow:"auto",padding:"0 16px 20px"}}>
            {outRows.length===0?<div style={{padding:"40px 20px",textAlign:"center",color:"#484F58"}}><div style={{fontSize:40,marginBottom:8}}>📤</div>出庫履歴がありません<div style={{fontSize:13,marginTop:8}}>「+ 出庫を記録」ボタンから出庫を記録すると在庫数が自動で減ります</div></div>:(
              isMobile?(
                <div>{outRows.map(o=>(
                  <div key={o.id} style={{background:"#21262D",border:"1px solid #30363D",borderRadius:10,padding:14,marginBottom:10}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                      <span style={{fontFamily:"monospace",fontSize:12,color:"#8B5CF6",fontWeight:600}}>📤 {o.date}</span>
                      <button style={btnD} onClick={()=>{setOutgoing(out=>out.filter(x=>x.id!==o.id));addToast("削除しました","info");}}>削除</button>
                    </div>
                ...
                <div style={{background:"#21262D",borderRadius:8,border:"1px solid #30363D",overflow:"hidden",marginTop:4}}>
                  <table style={{width:"100%",borderCollapse:"collapse",minWidth:700}}>
                    <thead><tr style={{background:"#1C2128"}}>{["出庫日","商品名","JANコード","出庫数","販売単価","合計売上","販売先・出荷先","備考",""].map(h=><th key={h} style={thS}>{h}</th>)}</tr></thead>
                    <tbody>{outRows.map(o=>(
                      <tr key={o.id}>
                        <td style={{...tdS,fontFamily:"monospace",color:"#8B5CF6",fontWeight:600,whiteSpace:"nowrap"}}>{o.date}</td>
                        ...
                        <td style={{...tdS,color:"#484F58",maxWidth:120}}>{o.note||"—"}</td>
                        <td style={tdS}><button style={btnD} onClick={()=>{setOutgoing(out=>out.filter(x=>x.id!==o.id));addToast("削除しました","info");}}>削除</button></td>
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
      ...
      {tab==="dashboard"&&(
        <div style={{padding:16,display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            <div style={{background:"#21262D",border:"1px solid #30363D",borderRadius:8,padding:14}}>
              <div style={{fontSize:11,color:"#484F58",marginBottom:4}}>今月の入庫合計</div>
              <div style={{fontFamily:"monospace",fontSize:20,fontWeight:600,color:"#3FB950"}}>{fmtY(incoming.filter(h=>h.date.slice(0,7)===today().slice(0,7)).reduce((s,h)=>s+h.totalCost,0))}</div>
            </div>
            <div style={{background:"#21262D",border:"1px solid #30363D",borderRadius:8,padding:14}}>
              <div style={{fontSize:11,color:"#484F58",marginBottom:4}}>今月の出庫合計売上</div>
              <div style={{fontFamily:"monospace",fontSize:20,fontWeight:600,color:"#58A6FF"}}>{fmtY(outgoing.filter(o=>o.date.slice(0,7)===today().slice(0,7)).reduce((s,o)=>s+o.totalPrice,0))}</div>
            </div>
          </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={e=>{if(e.target===e.currentTarget)setEditModal(null);}}>
          <div style={{background:"#161B22",border:"1px solid #444C56",borderRadius:10,width:"100%",maxWidth:480,boxShadow:"0 20px 60px rgba(0,0,0,.5)",maxHeight:"90vh",overflowY:"auto"}}>
            ...
            <div style={{padding:"14px 16px",display:"flex",flexDirection:"column",gap:10}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                <div><div style={{fontSize:11,color:"#8B949E",marginBottom:4}}>単価（円）</div><input style={inpS} type=
