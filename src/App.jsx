...
// 画像を自前サーバー経由で取得することで、外部サイトのホットリンク対策を回避する
...
function ItemCard({item,cats,onEdit,onDelete,isAdmin}){
  ...
  return(<div style={{background:"#21262D",border:"1px solid #30363D",borderRadius:10,padding:14,marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}><div style={{display:"flex",flex:1,minWidth:0,marginRight:8,gap:10}}>{item.imageUrl?<img src={proxyImg(item.imageUrl)} alt="" style={{width:44,height:44,objectFit:"contain",borderRadius:6,background:"#fff",flexShrink:0}} onError={e=>{e.target.style.display="none";}}/>:null}<div style={{flex:1,minWidth:0}}><div style={{fontWeight:600,fontSize:14,lineHeight:1.4,marginBottom:2}}>{item.name}</div><div style={{fontFamily:"monospace",fontSize:10,color:"#484F58"}}>{item.jan}</div></div></div><span style={{flexShrink:0,background:badge.bg,color:badge.col,padding:"3px 8px",borderRadius:6,fontSize:11,fontWeight:700}}>{badge.text}</span></div>{(l1||l3)&&<div style={{fontSize:11,color:"#8B949E",marginBottom:8}}>{l1&&<span>{l1.emoji} {l1.name}</span>}{l3&&<span style={{color:"#484F58"}}> › {l3.name}</span>}</div>}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:8}}><div style={{background:"#1C2128",borderRadius:6,padding:"8px 10px"}}><div style={{fontSize:9,color:"#484F58",marginBottom:3}}>単価</div><div style={{fontFamily:"monospace",fontWeight:700,color:"#58A6FF",fontSize:14}}>{fmtY(item.price)}</div></div><div style={{background:"#1C2128",borderRadius:6,padding:"8px 10px"}}><div style={{fontSize:9,color:"#484F58",marginBottom:3}}>在庫数</div><div style={{fontFamily:"monospace",fontWeight:700,fontSize:20}}>{item.qty}</div></div><div style={{background:"#1C2128",borderRadius:6,padding:"8px 10px"}}><div style={{fontSize:9,color:"#484F58",marginBottom:3}}>粗利率</div><div style={{fontFamily:"monospace",fontWeight:700,color:mCol(m),fontSize:14}}>{m!==null?m+"%":"—"}</div></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,fontSize:11,color:"#8B949E",marginBottom:6}}><div>仕入れ値: <span style={{fontFamily:"monospace",color:"#E6EDF3"}}>{item.cost?fmtY(item.cost):"—"}</span></div><div>発注点: <span style={{fontFamily:"monospace",color:"#E6EDF3"}}>{item.reorderPoint||0}</span></div>{item.maker&&<div>メーカー: <span style={{color:"#E6EDF3"}}>{item.maker}</span></div>}{item.supplier&&<div>仕入れ先: <span style={{color:"#E6EDF3"}}>{item.supplier}</span></div>}{item.location&&<div>📍 場所: <span style={{color:"#58A6FF",fontWeight:600}}>{item.location}</span></div>}</div><div style={{display:"flex",gap:8,marginTop:10}}><button style={{flex:1,background:"rgba(88,166,255,.1)",color:"#58A6FF",border:"1px solid rgba(88,166,255,.3)",borderRadius:6,cursor:"pointer",fontSize:13,padding:"8px 0",fontWeight:600}} onClick={()=>onEdit(item)}>✏ 編集</button>{isAdmin&&<button style={{background:"transparent",color:"#F85149",border:"1px solid rgba(248,81,73,.3)",borderRadius:6,cursor:"pointer",fontSize:13,padding:"8px 14px"}} onClick={()=>onDelete(item.id)}>削除</button>}</div></div>);
}
...
export default function App(){
  ...
  async function saveEdit(id,field,val){
    ...
    const colMap={price:"price",cost:"cost",qty:"qty",reorderPoint:"reorder_point",name:"name",jan:"jan",maker:"maker",supplier:"supplier",location:"location"};
                ...
                <div style={{background:"#21262D",borderRadius:8,border:"1px solid #30363D",overflow:"hidden",marginTop:4}}>
                  <table style={{width:"100%",borderCollapse:"collapse",minWidth:1000}}>
                    ...
                    <tbody>{rows.map(item=>{const m=calcM(item.price,item.cost);const isAl=item.qty<=(item.reorderPoint||0);const l1=findL1(cats,item.catL1);const l3=findL3(cats,item.catL1,item.catL2,item.catL3);return(
                      <tr key={item.id}>
                        <td style={tdS}>{item.imageUrl?<img src={proxyImg(item.imageUrl)} alt="" style={{width:32,height:32,objectFit:"contain",borderRadius:4,background:"#fff"}} onError={e=>{e.target.style.display="none";}}/>:<span style={{color:"#484F58",fontSize:11}}>—</span>}</td>
